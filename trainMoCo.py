...
if __name__ == '__main__':
    ...
    train = load_words_mapping(args.train_entity_path)
    dev = load_words_mapping(args.dev_entity_path)
    test = load_words_mapping(args.test_entity_path)
    en_word2context = load_word2context_from_tsv_hiki(args.src_context_path,args.all_sentence_num)
    lg_word2context = load_word2context_from_tsv_hiki(args.trg_context_path,args.all_sentence_num)
    dev_dataset = WordWithContextDatasetWW(dev, en_word2context, lg_word2context,prepend_bos=with_span_eos,append_eos=with_span_eos,sampleNum=args.dev_sample_num,
        max_len=args.sentence_max_len,cut_type=args.cut_type)
    train_dataset = WordWithContextDatasetWW(train, en_word2context, lg_word2context,prepend_bos=with_span_eos,append_eos=with_span_eos,sampleNum=args.train_sample_num,
        max_len=args.sentence_max_len,cut_type=args.cut_type)
    ...
    if args.simclr == 1 or args.dev_only_q_encoder == 1:
        test_acc = test_model_simclr(model,test_loader)
    else:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if args.simclr == 1 or args.dev_only_q_encoder == 1:
            val_acc = test_model_simclr(model,val_loader)
        else:
        ...
        if args.local_rank == 1 or args.local_rank == -1:
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                if args.distributed:
                    torch.save(model.state_dict(),args.output_model_path)  # 直接存成多卡
                else:
                    torch.save(model.state_dict(),args.output_model_path)
    if args.local_rank == 1 or args.local_rank == -1:
        model.load_state_dict(torch.load(args.output_model_path))
        if args.simclr == 1 or args.dev_only_q_encoder == 1:
            val_acc = test_model_simclr(model,test_loader)
        else:
